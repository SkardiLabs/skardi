...
//! because [`MockFeedServer`] binds loopback and production's
//! `EgressPolicy::default_deny` refuses loopback like any other reserved range.
//! An external test crate cannot reach a `pub(crate)` item, and widening the
//! seam would be exactly the production-reachable private-network switch the
//! design forbids.
//!
