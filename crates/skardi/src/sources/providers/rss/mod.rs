...
#[cfg(feature = "rss")]
pub mod opml;
// The fetcher's egress-policy seam: an `EgressPolicy` trait consulted for
// every resolved address, with `AllowAll` as the only OSS implementation
// (no destination filtering by default — see the module doc). Constraining
// egress is delegated to the operator or an injected policy; OSS ships no
// default-deny guard. Not `pub` — it is an internal implementation detail of
// the fetch engine (`fetch` consumes it via `super::egress`), not part of
// this provider's public surface. Gated behind `rss` alongside the rest of
// the fetch/parse engine, even though its own dependencies (reqwest, tokio)
// are already unconditional crate deps.
#[cfg(feature = "rss")]
mod egress;
