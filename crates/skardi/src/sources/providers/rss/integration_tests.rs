...
//! `#[cfg(test)] pub(crate)`: it takes the loopback-allowing [`EgressPolicy`]
//! that the mock server needs, because the mock binds `127.0.0.1` and
//! production's `EgressPolicy::default_deny` refuses loopback like any other
//! reserved range. An external test crate cannot reach a `pub(crate)` item, and
//! widening the seam — a public entry point taking a policy or an
//! `allow_loopback` flag — would be exactly the production-reachable
//! private-network switch the design forbids. `open_connector`'s own
...
impl TestNews {
    ...
    ///
    /// A `target` beginning with `http://` or `https://` is used verbatim — the
    /// shape the egress tests need, since a refused address is by definition not
    /// on the mock server. Anything else is a path on the mock server, and gets
    ...
    async fn start(feeds: &[(&str, &str)], tune: impl FnOnce(&mut RssConfig)) -> Self {
        ...
        register_rss_tables_with_policy(
            ...
            // The mock binds loopback, which the production policy refuses.
            // Private/link-local/CGNAT stay blocked under this policy too,
            // which is what the AC15 tests below rely on.
...
// ---------------------------------------------------------------------------
// AC15 — reserved ranges are refused before connecting
// ---------------------------------------------------------------------------

/// Both entry points into the egress guard, under the *test* policy — which
/// relaxes loopback only, so private and link-local stay refused exactly as
/// under `default_deny`.
///
/// `blocked` names a private address directly and never reaches a socket at
/// all. `via_redirect` reaches the mock, which answers `302` to a private
/// address; the redirect target is validated before the next request is built,
/// so exactly one request is recorded — the pre-redirect one.
#[tokio::test]
async fn ac15_reserved_range_refused_direct_and_redirect() {
    let news = TestNews::start(
        &[
            ("healthy", "/healthy.xml"),
            ("blocked", "http://192.168.7.7/f.xml"),
            ("via_redirect", "/redirect.xml"),
        ],
        |_| {},
    )
    .await;
    news.script(
        "/redirect.xml",
        FeedScript::always(Canned::status(302).with_header("location", "http://10.9.9.9/f.xml")),
    );
    ...
    assert_eq!(
        col(&items, "feed"),
        vec!["healthy"],
        "a refused subscription contributes no rows, and does not fail the scan"
    );

    let feeds = news
        .sql("SELECT name, last_status, http_status, last_error FROM news.main.feeds ORDER BY name")
        .await;
    assert_eq!(
        col(&feeds, "name"),
        vec!["blocked", "healthy", "via_redirect"]
    );
    assert_eq!(
        col(&feeds, "last_status"),
        vec!["error", "fresh", "error"],
        "no window was ever cached for either refused feed"
    );
    assert_eq!(
        u16_col(&feeds, "http_status"),
        vec![None, Some(200), None],
        "a refusal happens before any status is observed"
    );
    let errors = opt_col(&feeds, "last_error");
    assert_eq!(
        errors[0].as_deref(),
        Some("egress blocked: host '192.168.7.7' resolves to private address 192.168.7.7"),
        "the direct case names the address and the range it broke"
    );
    assert_eq!(errors[1], None);
    assert_eq!(
        errors[2].as_deref(),
        Some("egress blocked: host '10.9.9.9' resolves to private address 10.9.9.9"),
        "the redirect case names the *target*, not the URL that was subscribed"
    );

    assert_eq!(
        sorted(news.paths()),
        vec!["/healthy.xml", "/redirect.xml"],
        "the private address is never connected to, on either path"
    );
}

/// The canonical case the design names, in a test whose failure reads as what
/// it is: cloud instance metadata at `169.254.169.254`, the address a
/// prompt-injected agent would add as a "feed".
#[tokio::test]
async fn ac15_cloud_metadata_address_is_refused() {
    let news = TestNews::start(
    ...
    let items = news.sql("SELECT guid FROM news.main.items").await;
    assert_eq!(total_rows(&items), 0);

    let feeds = news
        .sql("SELECT last_status, last_error FROM news.main.feeds")
        .await;
    assert_eq!(col(&feeds, "last_status"), vec!["error"]);
    assert_eq!(
        opt_col(&feeds, "last_error"),
        vec![Some(
            "egress blocked: host '169.254.169.254' resolves to link-local address 169.254.169.254"
                .to_string()
        )]
    );
    assert!(
        news.requests().is_empty(),
