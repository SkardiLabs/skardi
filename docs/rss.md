...
cloud instance metadata on its behalf.

**Skardi OSS does not sandbox fetch destinations.** A subscribed feed URL — or
a redirect it issues — reaches any address the host can route to, including
link-local `169.254.169.254` and RFC 1918 private ranges. There is no reserved-
range refusal shipped in OSS: the fetcher's egress check exists as a seam
(`EgressPolicy`), but the constructor every production code path uses is
`AllowAll`, which permits everything. This is an SSRF exposure the operator
owns, not something Skardi filters on your behalf.

Given that, an operator should:

- impose egress control at the infrastructure layer — an egress firewall or
  `NetworkPolicy` denying RFC 1918 / link-local / cloud metadata destinations
  from the process that runs Skardi, plus IMDSv2 with a hop limit on any cloud
  instance it runs on — and/or
- inject a destination-filtering `EgressPolicy` at the call site that
  constructs the RSS engine, if embedding Skardi as a library.

Managed, default-on egress governance (the reserved-range refusal this section
used to describe) is a **Skardi Cloud** feature, not an OSS one — see the
[Cloud egress design](superpowers/specs/2026-08-03-rss-cloud-egress-design.md)
for the policy Cloud applies.

The seam still shapes two properties worth knowing, whichever policy ends up
installed:

- **Redirects are re-checked.** Every hop's target is validated against the
  active policy before the next request is built, so a policy that refuses a
  target catches it on a redirect hop too — the recorded error names the
  *target*, not the URL that was subscribed.
- **A mixed answer fails the whole lookup.** If one hostname resolves to both a
  refused and an allowed address under a denying policy, the resolution is
  refused entirely rather than narrowed to the address that passed.

A subscription refused by an installed policy degrades exactly like an
unreachable one: `last_status = 'error'`, `http_status` `NULL` (no status was
ever observed), zero rows in `items`, other feeds unaffected. `last_error`
reads:
...
Under the OSS default this never fires — `AllowAll` refuses nothing, so no
subscription in OSS degrades this way unless you have injected your own
denying `EgressPolicy`.
...
supplies one of those rails outright — subscription edits are configuration-only
and human-visible — but not the other: OSS does not cap a hijacked agent's
network reach by default (see [Egress policy](#egress-policy-ssrf)), so that
containment is the operator's to add, at the infrastructure layer or via an
injected `EgressPolicy`.
...
|---|---|---|
| `egress blocked: …` | Egress policy, before connect | An installed `EgressPolicy` refused the resolved address, on the initial URL or a redirect hop. Never produced under the OSS default (`AllowAll`) — only reachable if an operator has injected a denying policy. The message names the address and the reason given. Not a network problem — see [Egress policy](#egress-policy-ssrf). |
