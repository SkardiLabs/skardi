...
- **Sanitation is conservative by contract:** each repair rung is a byte-level no-op on well-formed input; rungs apply cumulatively; applied rungs are recorded in `conformance_notes`. Rungs run *before* the parse, not as a failure-triggered fallback — see decision 14.
...
- **Every server-supplied number that becomes a wait, a size, a loop count, or a recursion depth gets a local ceiling.** A remote value is attacker input: `Retry-After: 999999` is one header and eleven days of `tokio::time::sleep`. Clamp at the point of use even when an outer deadline is planned elsewhere — the outer bound only helps if it cancels, which is a separate task's promise. `parse_retry_after`'s own doc assigns the cap to its caller, and `open_connector/client.rs:87` (`MAX_RETRY_WAIT`) is the in-repo precedent.
...
- **A comment may not state a technical claim its author has not verified.** The operational rule: whenever you write *because X* or *since X* about how a library, a mask, or a protocol behaves, either open the source or doc and cite what you read, or delete the causal clause and state only the observable effect. The shorter true sentence is always acceptable.
- **Do not enumerate a dependency's taxonomy to establish a property; enforce the property with a test and cite what you measured.** A comment claiming to list every error a library can surface, every range a parser accepts, or every state a type can reach is asserting exhaustiveness over code you do not own and that changes on upgrade. Three consecutive attempts to state one such enumeration correctly in this plan each shipped a wrong claim — the third inside the paragraph rewritten to fix the first two. Instead: state the property, name the test that enforces it, and record what was observed at which pinned versions as evidence rather than as a closed list. Then an upgrade that changes behavior fails a test instead of silently falsifying a comment. Four review findings so far were confident wrong explanations — a stdlib predicate called unstable when it has been stable since Rust 1.7; a design choice attributed to named vendors' OPML exports nobody had examined; a test case said to discriminate against a bitmask that still matches it; and `content_length()` described as reading a header it never reads. Each cost a review round. In security- and protocol-facing code a wrong comment outlives the reader who could have caught it, and sends the next auditor hunting for a mechanism that does not exist.
- **A test must fail loudly, not hang.** If the bug a test guards against manifests as an unbounded wait, the test cannot assert its way out of it — under the regression the assertion is never reached and CI reports a suite timeout instead of a named failure. Drive such tests with `#[tokio::test(start_paused = true)]` so tokio's clock auto-advances, and assert on virtual elapsed time. Keeping real-time waits in the suite also compounds: one 10-second sleep is the whole suite's runtime for every later task.
...
5. **Entries with neither `id` nor `link` are skipped**, counted in a `entries-without-identity: <n>` conformance note (a `(feed, guid)` key cannot be null). Requires injecting an id generator — see decision 15.
...
14. **Sanitation runs before the parse, not as a failure-triggered ladder** (approved during Task 7; deviates from the spec's "stop at the first rung that parses" wording). Measured against `feed-rs` 2.4: it does not reject malformed lexis, it succeeds while silently discarding the offending element — a single naked `&` in `<title>Fish & Chips</title>` yields `Ok` with `title: None`, and an illegal `0x08` survives into the parsed string. A ladder gated on `Err` therefore never fires on the documents it exists to rescue. Applying the rungs up front is safe precisely because of the AC16 conservativeness contract (each rung is a byte-level no-op on well-formed input, pinned by `sanitize.rs` tests), so well-formed documents are byte-identical and parse unchanged. `parse.rs` carries a regression guard that fails if a future `feed-rs` starts erroring or starts preserving the field on its own.
15. **`feed-rs` is built with an id generator that yields nothing** (`Builder::id_generator(|_,_,_| String::new())`), discovered in Task 8. By default `feed-rs` fills a missing `entry.id` itself — from a content hash when a link exists, and from a **random UUID** when nothing does. The random case is unusable as identity: the same item would take a new `(feed, guid)` on every scan, breaking window identity and the idempotent-archive acceptance criterion. Both cases also mask the identity-less entry that decision 5 says to skip, making that rule dead code. Because the generator only runs when the id is empty, suppressing it leaves `entry.id` populated exactly when the feed really supplied one, so the Field Mapping chain (id → link → skip) decides identity deterministically. Pinned by `item_identity_is_deterministic_and_never_a_random_uuid`.
16. **Enclosures come from `media` *or* `links`, by dialect** (measured in Task 8, correcting this plan's earlier claim that `feed-rs` folds all three into `media`). Only RSS 2.0 `<enclosure>`/MediaRSS reach `entry.media`; Atom `rel="enclosure"` links and JSON Feed attachments both stay in `entry.links`. A JSON attachment arrives `rel`-less but with a `media_type`, which is what separates it from the item's own URL — and because `rel`-less is also how that URL arrives, attachments must be excluded from `link`/`guid` candidacy too, or a podcast item with no `url` ends up linking to its own mp3. Pinned by `a_json_attachment_is_never_mistaken_for_the_item_link`. Also: the MIME accessor on `MediaTypeBuf` is `subty()`, not `subtype()`.
...
- [ ] **Step 4: Implement `fetch.rs`.** Single client at construction (see Interfaces). `fetch()` algorithm:
  ...
  2. Redirect loop up to `MAX_REDIRECT_HOPS`: per hop, attempt loop up to `MAX_ATTEMPTS`:
     ...
     - Status 304 **and this hop actually sent a validator** → `NotModified`; an unconditional request answered `304` is a protocol violation and returns `Status { status: 304 }`. The qualifier is load-bearing: validators are suppressed after the first hop, and `fetch(url, None)` sends none at all, so an unguarded 304 mapping lets a hostile server pin a feed at "not modified" forever while the engine has no cached window to re-serve. Status 3xx with `Location` → resolve relative against the current hop's URL, then **re-run step 1's checks on the resolved target before the next request**: scheme allowlist, and `policy.check_ip` when the host is an IP literal. Hostname targets re-enter `PolicyDns` on their own; IP-literal targets do not — reqwest's connector (hyper-util `HttpConnector`) calls `SocketAddrs::try_parse(host, port)` first and never consults the custom resolver when the host parses as an address, bracketed IPv6 included. This per-hop check is the only thing standing between a public feed's `302` and `http://169.254.169.254/`. 429/500/502/503/504 → retryable. Other non-2xx → `Status` (final).
     - Retry wait: `min(max(parse_retry_after(&resp), backoff), MAX_RETRY_WAIT)` where backoff = `RETRY_BASE_BACKOFF_MS * 2^attempt` ± 50% jitter and `MAX_RETRY_WAIT: Duration = 10s` (reuse the open_connector client's jitter helper pattern — see `open_connector/client.rs`; if it is private, copy the two-line implementation, do not refactor OC in this task). The ceiling is not optional: `Retry-After` is server-supplied, so an unclamped `max()` hands a hostile feed an arbitrary sleep. `open_connector/client.rs:87` is the precedent.
...
- Produces (`parse.rs`):
  ...
  - `pub fn parse_with_ladder(bytes: &[u8]) -> Result<ParseSuccess, ParseFailure>` — family-detect; `refuse_internal_dtd` (XML only) → apply the applicable rungs cumulatively → parse once. XML applies rungs 1+2+3; JSON applies only rung 1 (rungs 2-3 repair XML lexis and a naked `&` is legal inside a JSON string). `repairs` records exactly the rungs whose cumulative application changed bytes (`changed == true`); each change and the parse outcome are traced at `debug` level. Rungs run before the parse rather than after a failure — see decision 14.
...
- `link` = first `entry.links` with `rel == None || rel == Some("alternate")`, else first link; store `href` as-is. **Attachments are excluded from both fallbacks** — see decision 16: a JSON Feed attachment arrives as a `rel`-less link, so an item whose `url` is absent would otherwise promote its audio/video attachment to `link` and, through the identity chain, to `guid`. An attachment is `rel == "enclosure"` or (`rel`-less with a `media_type` set).
...
2. Expired → `let _permit = semaphore.acquire().await` (politeness; released on drop, incl. cancellation) → **if `!launch_gate()` return `None`** (LIMIT filled while waiting; no fetch, no health write) → `fetcher.fetch(url, window validators)`:
   ...
   - `Ok(Fetched { body, .. })` → `parse_feed_document(&body, content_type)`:
     ...
     - `Err(pf)` → treat as failure with `error = truncate(format!("parse failed at {}: {}", pf.stage, pf.reason), 512)` — truncate the *composed* string, not the inner reason, or the stored value exceeds the stated cap by the prefix's length; **also** fold `pf.dialect_declared` into the recorded observation (the sniff worked even though parse failed). Recording that sniff needs a writer: `FeedObservation.dialect_declared` exists from Task 10 with no method setting it, so `record_failure` takes it as a parameter (`Some` overwrites, `None` preserves) — the alternative, a second per-feed map in the engine, would give one `feeds` column two sources of truth.
   - `Err(fe)` → failure with `error = truncate(fe.to_string(), 512)`, `http_status` from `Status{status}` variants.
   - **Error redaction is this task's job, and truncation is not redaction.** The global constraint requires `last_error` to carry no response-body content; `truncate(_, 512)` only bounds length. Audited against the pinned `feed-rs` 2.4 / `quick-xml` 0.41 (the pair feed-rs actually resolves to — an earlier draft of this line said 0.38, which was wrong), a parse error's Display can echo *structural* fragments of the document — `ParseErrorKind::UnknownMimeType(String)` and `JsonUnsupportedVersion(String)` quote a MIME/version string from the source, and `quick-xml`'s ill-formed variants quote element names (e.g. a mismatched end tag). Decision: element names, MIME strings, and version strings are kept — they are what makes a malformed feed diagnosable — while the 512-char cap bounds the worst case. **State the property with that exception, not as a blanket claim.** "No character data or attribute values reach the column" is technically true and materially misleading, because a JSON Feed's declared `version` *is* attacker-supplied body text and does reach it verbatim (`feed-rs/src/parser/mod.rs:66`, measured) — it is simply neither character data nor an attribute. Two tests are required: one asserting a sentinel embedded in body character data never appears in `last_error` (so a future `feed-rs`/`quick-xml` that starts echoing it fails a test rather than silently widening the exception), and one driving the known exception from its own side with an oversized version string, asserting the stored value is capped — that second test is the only thing demonstrating the cap is what bounds an attacker-supplied field reaching this column.
...
cargo doc -p skardi --features rss --no-deps   # rustdoc lints; check does NOT run them
...
**Per-task verification must include `cargo doc`, not just `cargo check`.** They lint different things: `check` never runs rustdoc, so a broken intra-doc link — `[SomethingPrivate]` in a `pub` item's docs, the commonest form — passes `check` cleanly and only shows up in a doc build. This plan reported "zero warnings" for eleven tasks on the strength of `cargo check` alone; the first `cargo doc` run surfaced six `private_intra_doc_links` warnings that had accumulated unnoticed. Run both, per task.
