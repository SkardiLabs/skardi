...
| URL refused by an injected egress policy | Only when a non-default `EgressPolicy` is wired (OSS default `AllowAll` refuses nothing): fetch refused before connect, on the initial URL or a redirect target; `feeds.last_status = 'error'`, `last_error` names the denial reason; zero rows in `items` even if a cached window existed — a policy verdict is not a transient fault to serve stale through, so the denial purges the window rather than serving it `stale-error` like a dead feed; other feeds unaffected |
...
15. The fetcher's egress seam is honored: with the default `AllowAll` policy every routable address is fetched, and with a test-injected denying `EgressPolicy` the refused target — on the initial URL and on a redirect hop — yields `feeds.last_status = 'error'` with `last_error` naming the denial reason, zero rows in `items` — on a warm cache too: the denial purges the cached window rather than serving it stale — and other feeds unaffected. OSS ships `AllowAll`; the denying policy is test-only.
